...
- `python -m scripts.run --no-mutate` — audit + report + "would have fired" proposer preview (observation mode; no executors invoked)
...
- (done) Week-over-week trend suffixes on consent-gap and attribution-gap flags — the renderer reads the prior snapshot via `AuditReport.prev_snapshot` and appends `(Xpp w/w)` when the delta crosses a 1pp noise floor.
