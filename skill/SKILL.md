...
Meta `create_campaign` is supported and has an auto-proposer: list the campaigns you want under `guardrails.proposer.desiredCampaigns` in brand.json (each needs `platform: "meta"`, `name`, `objective`; optional `dailyBudget`, `specialAdCategories`). Each audit reconciles that list against the account's full campaign inventory and proposes creating any that are missing — always PAUSED, always through the guardrails (so `newCampaignRequiresApproval` still gates them). The match is by name against every campaign in the account, not just those with delivery, so a paused campaign created last run is never recreated; if the inventory can't be fetched, nothing is proposed. You then populate ad sets/creative and launch the campaign yourself.
