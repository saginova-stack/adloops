...
Meta `create_campaign` is supported and has an auto-proposer: list the campaigns you want under `guardrails.proposer.desiredCampaigns` in brand.json (each needs `platform: "meta"`, `name`, and a valid ODAX `objective` — validated at load time; optional `dailyBudget`, `specialAdCategories`, and an `adSet` block). Each audit reconciles that list against the account's full campaign inventory:
